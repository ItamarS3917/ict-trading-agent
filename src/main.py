...
def run_backtest(
    symbol: str, start_date: str, end_date: str, initial_capital: float = 10000
) -> dict:
    """Run backtesting analysis."""
    print(f"🧪 Running backtest for {symbol} from {start_date} to {end_date}...")

    backtester = Backtester(initial_capital=initial_capital)
    results = backtester.run_backtest(symbol=symbol, start_date=start_date, end_date=end_date)
...
def main():
    ...
    parser.add_argument("--analyze", action="store_true", help="Analyze current market conditions")
    ...
    parser.add_argument("--symbol", type=str, default="NQ=F", help="Trading symbol (default: NQ=F)")
