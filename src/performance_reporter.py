...
class PerformanceReporter:
    ...
    def generate_monthly_report(self, trades: list[dict], equity_curve: list[dict]) -> dict:
        """
    ...
    def generate_drawdown_report(self, equity_curve: list[dict]) -> dict:
        ...
        for i, value in enumerate(equity):
            if value > peak:
                ...
                if (
                    current_dd["trough"] > 0
                    and current_dd["trough_value"] < current_dd["peak_value"]
                ):
                    dd_pct = (current_dd["peak_value"] - current_dd["trough_value"]) / current_dd[
                        "peak_value"
                    ]
                    drawdowns.append(
                        {
    ...
    def generate_risk_metrics(
        ...
        annual_return = (df_equity["equity"].iloc[-1] / initial_capital) ** (
            252 / len(df_equity)
        ) - 1
    ...
    def _analyze_time_performance(self, df: pd.DataFrame) -> dict:
        ...
        return {"hourly_performance": hourly, "daily_performance": daily}
    ...
    def export_report(
        self, report: dict, format: str = "json", filepath: Optional[str] = None
    ) -> str:
        """
