...
class Backtester:
    ...
    def __init__(
        self, initial_capital: float = 10000, commission: float = 2.0, slippage: float = 0.001
    ):
        """
    ...
    def _check_exit_signal(
        self, df: pd.DataFrame, position: dict, current_price: float
    ) -> Optional[str]:
        """
    ...
    def _calculate_position_size(
        self, capital: float, entry_price: float, stop_loss: float, risk_per_trade: float = 0.02
    ) -> int:
        """
    ...
    def _calculate_sharpe_ratio(
        self, equity_curve: list[dict], risk_free_rate: float = 0.02
    ) -> float:
        """
        ...
        for i in range(1, len(equity_curve)):
            ret = (equity_curve[i]["equity"] - equity_curve[i - 1]["equity"]) / equity_curve[i - 1][
                "equity"
            ]
            returns.append(ret)
