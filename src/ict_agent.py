...
class ICTTradingAgent:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        """
        self.config = self._normalize_config(config or self._default_config())
    ...
    def analyze_market_structure(self, symbol: str) -> dict:
        ...
        """
        # Get price data
        df = self.data_handler.get_price_data(symbol, period=f"{self.config['lookback_period']}d")
    ...
    def _identify_swing_points(self, df: pd.DataFrame, window: int = 5) -> dict:
        ...
        for i in range(window, len(df) - window):
            ...
            if all(
                ...
                highs.append({"index": i, "price": df.iloc[i]["High"], "timestamp": df.index[i]})
            ...
            if all(
                ...
                lows.append({"index": i, "price": df.iloc[i]["Low"], "timestamp": df.index[i]})
    ...
    def _is_valid_orderblock_signal(self, orderblock: dict, market_structure: dict) -> bool:
        ...
        return bool(
            trend == "UPTREND"
            and orderblock["direction"] == "BULLISH"
            or trend == "DOWNTREND"
            and orderblock["direction"] == "BEARISH"
        )
