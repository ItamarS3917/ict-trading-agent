...
def main():
    ...
    position_size = risk_manager.calculate_position_size(
        capital=capital, entry_price=entry_price, stop_loss=stop_loss
    )
    ...
    # Long position
    print("\nLong Position:")
    sl_long = risk_manager.calculate_stop_loss(entry_price=entry_price, direction="LONG", atr=atr)
    tp_long = risk_manager.calculate_take_profit(
        entry_price=entry_price, stop_loss=sl_long, direction="LONG"
    )
    ...
    # Short position
    print("\nShort Position:")
    sl_short = risk_manager.calculate_stop_loss(entry_price=entry_price, direction="SHORT", atr=atr)
    tp_short = risk_manager.calculate_take_profit(
        entry_price=entry_price, stop_loss=sl_short, direction="SHORT"
    )
    ...
    new_signal = {"price": 15000, "stop_loss": 14900, "take_profit": 15200, "direction": "LONG"}

    print("\n📊 New Trade Evaluation:")
    is_valid, reason = risk_manager.validate_trade(new_signal, capital, positions)

    if is_valid:
        print("  ✅ Can accept new position")
    else:
        print("  ❌ Cannot accept new position")
    print(f"  Reason: {reason}")
